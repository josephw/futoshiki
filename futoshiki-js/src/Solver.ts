...
//                Solver.log_$LI$().fine("This may take an extremely long time");
            ...
            if(this.solve$org_kafsemo_futoshiki_Futoshiki$org_kafsemo_futoshiki_CellPos_A$int$org_kafsemo_futoshiki_Possibilities$java_math_BigInteger(f, blanks, 0, poss, BigInt(0))) {
                this.target.remainingPossibilities(BigInt(0));
            }
        }
        public solve$org_kafsemo_futoshiki_Futoshiki$org_kafsemo_futoshiki_CellPos_A$int$org_kafsemo_futoshiki_Possibilities$java_math_BigInteger(f : Futoshiki, blank : CellPos[], nb : number, poss : Possibilities, possibilitiesAfter : bigint) : boolean {
            if(!f.isValid()) {
                return true;
            }
            if(nb >= blank.length) {
            ...
            let possibilitiesForValue : bigint[] = (s => { let a=[]; while(s-->0) a.push(null); return a; })(f.getLength());
            let possibilities : Possibilities[] = (s => { let a=[]; while(s-->0) a.push(null); return a; })(f.getLength());
            let i : number = 0;
            ...
            for(let j : number = 0; j < i; j++) {
                ...
                remainingPossibilities = remainingPossibilities - possibilitiesForValue[j];

                let more : boolean = this.solve$org_kafsemo_futoshiki_Futoshiki$org_kafsemo_futoshiki_CellPos_A$int$org_kafsemo_futoshiki_Possibilities$java_math_BigInteger(f.clone(), blank, nb + 1, possibilities[j], remainingPossibilities);
                if(!more) {
                    return false;
                }
            }
...
//        }
        ...
        moveBlankWithLeastPossibilitiesIntoPlace(blanks : CellPos[], p : number, poss : Possibilities) : CellPos[] {
