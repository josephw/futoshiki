...
import { Futoshiki } from './Futoshiki';
    ...
    export class Possibilities extends Grid {
        ...
        public use$int$int$int(column : number, row : number, value : number) {
            if(value < 1 || value > this.length) throw new Error("Bad cell value " + value);
        ...
        public use(column : number | Futoshiki, row? : any, value? : any) : number {
            if(((typeof column === 'number')) && ((typeof row === 'number') || row === null) && ((typeof value === 'number') || value === null)) {
                return <any>this.use$int$int$int(column, row, value);
            } else if(((column instanceof Futoshiki)) && row === undefined && value === undefined) {
                ...
                for(let r of f.getRules()) {
                        ...
                        if(greatestMoreThan > 0) {
                            for(let v : number = 1; v <= greatestMoreThan; v++) {{
                                learnedSomething = !this.possibilities.has(this.bit(r.getGreaterColumn(), r.getGreaterRow(), v)) || learnedSomething;
                                this.possibilities.add(this.bit(r.getGreaterColumn(), r.getGreaterRow(), v));
                            };}
                        } else {
                            for(let v : number = 1; v <= this.length; v++) {{
                                learnedSomething = !this.possibilities.has(this.bit(r.getGreaterColumn(), r.getGreaterRow(), v)) || learnedSomething;
                        ...
                        if(leastLessThan > 0) {
                            for(let v : number = leastLessThan; v <= this.length; v++) {{
                                learnedSomething = !this.possibilities.has(this.bit(r.getLesserColumn(), r.getLesserRow(), v)) || learnedSomething;
                                this.possibilities.add(this.bit(r.getLesserColumn(), r.getLesserRow(), v));
                            };}
                        } else {
                            for(let v : number = 1; v <= this.length; v++) {{
                                learnedSomething = !this.possibilities.has(this.bit(r.getLesserColumn(), r.getLesserRow(), v)) || learnedSomething;
	...
	return p;
        ...
        public possibleCount(column : number | CellPos, row? : any) : number {
            if(((typeof column === 'number')) && ((typeof row === 'number') || row === null)) {
